...
export const InvitationCard = ({ invitation }: InvitationCardProps) => {
  const name = invitation.idEstablecimiento || 'Tambo General'
  ...
  return (
    <div className="max-w-sm w-full bg-white border border-emerald-800/30 rounded-[2rem] p-10 shadow-lg flex flex-col items-center text-center font-sans">
      ...
      <div className="mb-10">
        ...
        <h3 className="text-emerald-700 text-lg font-bold">{invitation.rol}</h3>
      </div>

      <div className="w-full space-y-3">
        <button className="w-full bg-[#2D8659] hover:bg-[#246d47] text-white font-bold py-4 rounded-xl transition-colors text-lg">
          Aceptar Invitación
        </button>

        <button className="w-full bg-[#96A3B6] hover:bg-[#808da1] text-white font-bold py-4 rounded-xl transition-colors text-lg">
          Rechazar
        </button>
      </div>
    </div>
